...
const TourCard = memo(function TourCard({ tour, index, siteSettings }: { tour: Tour; index: number; siteSettings: SiteSettings }) {
  ...
  return (
    ...
    >
      ...
      {/* Package Info Overlay - Cinematic Style */}
      <AnimatePresence>
        {showInfo && hasPackageInfo && (
          <motion.div
            ...
            className="absolute inset-0 bg-gradient-to-b from-slate-900/98 via-slate-900/98 to-black/98 backdrop-blur-xl rounded-2xl md:rounded-3xl overflow-hidden z-20"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Ambient gradient overlay */}
            <div className="absolute inset-0 bg-gradient-to-br from-cyan-600/10 via-transparent to-teal-600/10 pointer-events-none" />
            ...
            <motion.button
              ...
              className="absolute top-6 right-6 z-10 w-12 h-12 bg-white/10 hover:bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center transition-all duration-300 shadow-2xl border border-white/10 group"
            >
            ...
            {/* Scrollable content */}
            <div className="h-full overflow-y-auto p-8 md:p-12 pt-20 pb-32 relative z-10">
              <div className="max-w-2xl mx-auto space-y-8">
                ...
                >
                  <h4 className="text-4xl md:text-6xl font-black text-white leading-[0.95] tracking-tighter mb-4">
                    {tour.title}
                  </h4>
                  {tour.packageName && (
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-cyan-500 to-teal-500 rounded-2xl shadow-lg shadow-cyan-500/20">
                      <div className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
                      <span className="text-white font-black text-sm uppercase tracking-wide">{tour.packageName}</span>
                    </div>
                  )}
                </motion.div>
                ...
                {/* Price & Duration - Cinematic Cards */}
                {(tour.packagePrice || tour.packageDuration) && (
                  ...
                  >
                    {tour.packagePrice && (
                      <div className="p-6 bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-sm rounded-2xl border border-white/10 shadow-xl">
                        <p className="text-xs text-white/40 uppercase tracking-widest font-black mb-2">Precio</p>
                        <p className="text-4xl md:text-5xl font-black bg-gradient-to-r from-cyan-400 to-teal-400 bg-clip-text text-transparent">
                          {tour.packagePrice}
                        </p>
                      </div>
                    )}
                    {tour.packageDuration && (
                      <div className="p-6 bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-sm rounded-2xl border border-white/10 shadow-xl">
                        <p className="text-xs text-white/40 uppercase tracking-widest font-black mb-2">Duración</p>
                ...
                {tour.packageIncludes && tour.packageIncludes.length > 0 && (
                  <motion.div
                    ...
                    className="space-y-4"
                  >
                    <h5 className="text-xs text-white/40 uppercase tracking-widest font-black">Lo que incluye:</h5>
                    <div className="space-y-3">
                      {tour.packageIncludes.map((item, i) => (
                        <motion.div
                          ...
                          className="group flex items-start gap-4 p-4 bg-gradient-to-r from-white/5 to-transparent hover:from-cyan-500/10 hover:to-teal-500/5 backdrop-blur-sm rounded-xl border border-white/5 hover:border-cyan-500/30 transition-all duration-300"
                        >
                          <div className="flex-shrink-0 w-6 h-6 rounded-lg bg-gradient-to-br from-cyan-500 to-teal-500 flex items-center justify-center shadow-lg shadow-cyan-500/30 mt-1">
                            <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                            </svg>
                          </div>
                          <span className="text-base md:text-lg text-white/90 leading-relaxed font-light">{item}</span>
                        </motion.div>
                      ))}
                    </div>
                  </motion.div>
            ...
            {/* Sticky CTA - Cinematic */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.6 }}
              className="absolute bottom-0 left-0 right-0 p-6 md:p-8 bg-gradient-to-t from-black via-black/95 to-transparent backdrop-blur-sm"
            >
              <a
                ...
                onClick={(e) => e.stopPropagation()}
                className="block w-full max-w-2xl mx-auto px-8 py-5 bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-500 hover:to-teal-500 text-white rounded-2xl font-black text-base md:text-lg shadow-2xl shadow-cyan-500/30 hover:shadow-cyan-500/50 transition-all duration-300 group"
              >
